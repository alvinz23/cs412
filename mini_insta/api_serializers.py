...
class ProfileSerializer(serializers.ModelSerializer):
    # Basic profile fields we send to the app.
    class Meta:
        model = Profile
...
class PhotoSerializer(serializers.ModelSerializer):
    # We expose one final image field no matter where image came from.
    ...
    def get_image(self, obj):
        # Use helper from model to return URL string.
        return obj.get_image_url()


class PostSerializer(serializers.ModelSerializer):
    # Include related profile info so app can render quickly.
    ...
    def get_photos(self, obj):
        # Attach all photos for this post in API output.
        return PhotoSerializer(obj.get_all_photos(), many=True).data


class CreatePostSerializer(serializers.Serializer):
    # Input validator for creating posts from API.
